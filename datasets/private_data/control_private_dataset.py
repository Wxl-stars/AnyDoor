from private.private_multimodal import PrivateE2EDataset
import random
import numpy as np
...
class ControlPrivate(PrivateE2EDataset):
    view_colors = {
            'CAM_FRONT':[0, 130, 180],
    ...
    def __init__(self, resemble_size=(2,3), final_dim=(224, 400), *args, **kwargs):
        ...
        self.num_frames = self.num_frames_per_sample
    ...
    def draw_corners_new(self, target, boxes, labels, colors, trans_lidar2cam, trans_cam2pix, linewidth=4):
        # corners [n, 8, 2]
    ...
    def _prepare_one_frame(self, item):
        ...
        source_img = item['img'] #torch.Size([6, 3, 256, 704]) 
        source_label_3d = item['gt_labels_3d'] # torch.Size([32])  
        source_bbox_3d = item['gt_bboxes_3d'] # torch.Size([32, 9])
        ...
        for view_id, view in enumerate(self.img_key_list):
            ...
            source_corner = self.draw_corners_new(img, source_bbox_3d, source_label_3d, self.colors, lidar2cam[view_id], cam2pix[view_id], linewidth=2) ###for 512
        ...
        render_map = np.zeros_like(source[..., :3])
    ...
    def __getitem__(self, idx):
        ...
        sweep_targets = list()
        sweep_sources = list()
        for frame_idx in range(self.num_frames_per_sample):
            item = {
                "img": data["imgs"][frame_idx],
                "gt_labels_3d": data["gt_labels"][frame_idx],
                "gt_bboxes_3d": data["gt_boxes"][frame_idx],
                "lidar2imgs": data["lidar2imgs"][frame_idx],
                "ida_mats": data["ida_mats"][frame_idx],
                "lidar2cam": data["lidar2cam"][frame_idx],
                "cam2img": data["cam2img"][frame_idx],
                }
        ...
        # 时间顺序
        sweep_targets = sweep_targets.permute(3, 0, 1, 2) # 3, n_frames, H, W
        sweep_targets = sweep_targets #.flip(0)  # BGR->RGB

        sweep_sources = (sweep_sources.to(torch.float32) / 127.5) - 1.0
        sweep_sources = sweep_sources.permute(3, 0, 1, 2)
        prompt = self.generate_prompts(data["gt_labels"][key_frame_idx], data["tags"][key_frame_idx])
